...
public class GestionConexionesAeropuertosPorDia extends Problema {
	...
	public Individuo evaluate(Individuo ind) {
		 ...
		 * double sumaObj = 0;
		 * 
		 * for (int i = 1; i < 7; i++) { sumaObj += sumaObj; } sumaObj = sumaObj / 6;
		 */
		...
		/*
		 * sumaPesos += sumaObj * this.pesos.get(0); sumaPesos += objetivos.get(7) *
		 * this.pesos.get(1);
		 */
	...
	private List<Double> calcularObjetivosDia(int dia, Individuo ind) {
		...
		if (this.datos.getDatosPorDia().get(dia).getConexiones().size() + offset > ind.getVariables().size()) {
			...
			bitsDia = ind.getVariables().subList(offset,
					this.datos.getDatosPorDia().get(dia).getConexiones().size() + offset);
		}
	...
	private List<Double> calcularRiesgoPasajerosIngresosHPasajerosHIngresosDia(int dia, List<Double> variablesDia) {
		...
		// java.util.Map<String, Double> ingresosTAeropuertoTotal = new
		// java.util.HashMap<>();
		...
		for (int j = 0; j < this.datos.getDatosPorDia().get(dia).getCompanyias().size(); j++) {
			for (int i = 0; i < this.datos.getDatosPorDia().get(dia).getConexiones().size(); i++) {
				if (!calculado) {
					/*
					 * if(variablesDia.get(i) == 0.0) { vuelos_cancelados +=
					 * this.datos.getDatosPorDia().get(dia).getVuelosEntrantesConexion().get(this.
					 * datos.getDatosPorDia().get(dia).getConexiones().get(i)); }
					 */

					// Riesgo, Pasajeros e Ingresos -----------------------
					Riesgosumatorio += this.datos.getDatosPorDia().get(dia).getRiesgos().get(i) * variablesDia.get(i);
					...
					Ingresossuma += (this.datos.getDatosPorDia().get(dia).getDineroMedioT().get(i)
							+ this.datos.getDatosPorDia().get(dia).getDineroMedioN().get(i)) * variablesDia.get(i);
					IngresostotalSuma += this.datos.getDatosPorDia().get(dia).getDineroMedioT().get(i)
							+ this.datos.getDatosPorDia().get(dia).getDineroMedioN().get(i);

					Ingresostasassuma += this.datos.getDatosPorDia().get(dia).getTasas().get(i) * variablesDia.get(i);
					...
					/*
					 * if (ingresosTAeropuertoTotal.get(this.datos.getDatosPorDia().get(dia).
					 * getConexiones().get(i).get(1)) != null) {
					 * ingresosTAeropuertoTotal.put(this.datos.getDatosPorDia().get(dia).
					 * getConexiones().get(i).get(1), ingresosTAeropuertoTotal.get(
					 * this.datos.getDatosPorDia().get(dia).getConexiones().get(i).get(1)) +
					 * this.datos.getDatosPorDia().get(dia).getDineroMedioT().get(i)); //Sustituir
					 * por i } else {
					 * ingresosTAeropuertoTotal.put(this.datos.getDatosPorDia().get(dia).
					 * getConexiones().get(i).get(1),
					 * this.datos.getDatosPorDia().get(dia).getDineroMedioT().get(i)); //Sustituir
					 * por i }
					 */
					if (variablesDia.get(i) == 1.0) { // Descomentar la linea
						if (ingresosTAeropuertosVar
								.get(this.datos.getDatosPorDia().get(dia).getConexiones().get(i).get(1)) != null) {
							ingresosTAeropuertosVar.put(
									this.datos.getDatosPorDia().get(dia).getConexiones().get(i).get(1),
									ingresosTAeropuertosVar
											.get(this.datos.getDatosPorDia().get(dia).getConexiones().get(i).get(1))
											+ this.datos.getDatosPorDia().get(dia).getDineroMedioT().get(i)); // Sustituir
																												// por i
						} else {
							ingresosTAeropuertosVar.put(
									this.datos.getDatosPorDia().get(dia).getConexiones().get(i).get(1),
									this.datos.getDatosPorDia().get(dia).getDineroMedioT().get(i)); // Sustituir por i
						}
					}
					// Cálculo homogeneidad pérdida de ingresos entre aeropuertos destino mediante
					// las tasas
					if (!sumaTasasPorAeropuerto.keySet()
							.contains(this.datos.getDatosPorDia().get(dia).getConexiones().get(i).get(1))) {
						sumaTasasPorAeropuerto.put(this.datos.getDatosPorDia().get(dia).getConexiones().get(i).get(1),
								variablesDia.get(i) * this.datos.getDatosPorDia().get(dia).getTasas().get(i));
					} else {
						sumaTasasPorAeropuerto.put(this.datos.getDatosPorDia().get(dia).getConexiones().get(i).get(1),
								sumaTasasPorAeropuerto
										.get(this.datos.getDatosPorDia().get(dia).getConexiones().get(i).get(1))
										+ variablesDia.get(i) * this.datos.getDatosPorDia().get(dia).getTasas().get(i));
					}
				...
				if (this.datos.getDatosPorDia().get(dia).getPasajerosCompanyia()
						.get(List.of(this.datos.getDatosPorDia().get(dia).getConexiones().get(i).get(0),
								this.datos.getDatosPorDia().get(dia).getConexiones().get(i).get(1),
								this.datos.getDatosPorDia().get(dia).getCompanyias().get(j))) != null) {
					totalPasajerosCompanyias[j] = totalPasajerosCompanyias[j]
							+ this.datos.getDatosPorDia().get(dia).getPasajerosCompanyia()
									.get(List.of(this.datos.getDatosPorDia().get(dia).getConexiones().get(i).get(0),
											this.datos.getDatosPorDia().get(dia).getConexiones().get(i).get(1),
											this.datos.getDatosPorDia().get(dia).getCompanyias().get(j)));
					if (variablesDia.get(i) == 1.0) { // Descomentar la linea
						totalPasajerosConexiones[j] = totalPasajerosConexiones[j]
								+ this.datos.getDatosPorDia().get(dia).getPasajerosCompanyia()
										.get(List.of(this.datos.getDatosPorDia().get(dia).getConexiones().get(i).get(0),
												this.datos.getDatosPorDia().get(dia).getConexiones().get(i).get(1),
												this.datos.getDatosPorDia().get(dia).getCompanyias().get(j)));
			...
			if (totalPasajerosCompanyias[j] != 0) {
				...
				porcentajePerdidoMedia = porcentajePerdidoMedia + 1
						- (double) totalPasajerosConexiones[j] / totalPasajerosCompanyias[j];
			}
		}
		...
		for (String destino : this.datos.getDatosPorDia().get(dia).getAeropuertosDestino()) {
			if (this.datos.getDatosPorDia().get(dia).getTasasPorAeropuertoDestino().get(destino) == 0.0) {
				...
				porcentajePerdidoTasasPorAeropuerto.put(destino, 1 - sumaTasasPorAeropuerto.get(destino)
						/ this.datos.getDatosPorDia().get(dia).getTasasPorAeropuertoDestino().get(destino));
				porcentajePerdidoTasasMedia += 1 - sumaTasasPorAeropuerto.get(destino)
						/ this.datos.getDatosPorDia().get(dia).getTasasPorAeropuertoDestino().get(destino);
			}

		}
		porcentajePerdidoTasasMedia = porcentajePerdidoTasasMedia
				/ this.datos.getDatosPorDia().get(dia).getAeropuertosDestino().size();

		for (String destino : this.datos.getDatosPorDia().get(dia).getAeropuertosDestino()) {
			porcentajePerdidoTasasDesviacionMedia += Math
					.abs(porcentajePerdidoTasasPorAeropuerto.get(destino) - porcentajePerdidoTasasMedia);
		}
		porcentajePerdidoTasasDesviacionMedia = porcentajePerdidoTasasDesviacionMedia
				/ this.datos.getDatosPorDia().get(dia).getAeropuertosDestino().size();
		...
		for (String key : this.datos.getDatosPorDia().get(dia).getIngresosTAeropuertoTotal().keySet()) {
			if (ingresosTAeropuertosVar.get(key) != null) {
				IporcentajePerdido.add(1 - (double) ingresosTAeropuertosVar.get(key)
						/ this.datos.getDatosPorDia().get(dia).getIngresosTAeropuertoTotal().get(key));
	...
	private Double calculoConectividadDia(int dia, List<Double> variablesDia/* , Individuo solution */) {
		...
		for (String origen : this.datos.getDatosPorDia().get(dia).getAeropuertosOrigen()) {
			...
			for (String destino : this.datos.getDatosPorDia().get(dia).getListaConexionesSalidas().get(origen)) {
				auxSuma += variablesDia
						.get(Utils.encontrarIndiceEnLista(this.datos.getDatosPorDia().get(dia).getConexiones(),
								List.of(origen, destino)))
						* this.datos.getDatosPorDia().get(dia).getVuelosEntrantesConexion()
								.get(List.of(origen, destino));
				auxTotalSuma += this.datos.getDatosPorDia().get(dia).getVuelosEntrantesConexion()
						.get(List.of(origen, destino));
			}
			Double aux = 0.0;
			if (auxTotalSuma != 0) {
	...
	public Map<String, String> calcularValoresAdicionales(Individuo ind) {
		...
		for (int i = 0; i < this.datos.getDatosPorDia().size(); i++) {
			...
			if (this.datos.getDatosPorDia().get(i).getConexiones().size() + offset > ind.getVariables().size()) {
				...
				bitsDia = ind.getVariables().subList(offset,
						this.datos.getDatosPorDia().get(i).getConexiones().size() + offset);
			}

			for (int k = 0; k < this.datos.getDatosPorDia().get(i).getConexiones().size(); k++) {
				if (bitsDia.get(k) == 0.0) {
					vuelos_cancelados += this.datos.getDatosPorDia().get(i).getVuelosEntrantesConexion()
							.get(this.datos.getDatosPorDia().get(i).getConexiones().get(k));
				}

...
public class MutationOperator {
	...
	public MutationOperator(double probMuta, double indiceDistr) {
		super();
	...
	//Mutación polinómica
	public Particle polyMut(Particle solucion, Problem prob) {
	...
	public Particle cambioDeBit(Particle solucion, Problem prob) {
		List<Double> solucionVal = solucion.getVariables();
	...
	//Mutación cambio de bit
		public Particle numeroAleatorio(Particle solucion, Problem prob) {
			List<Double> solucionVal = solucion.getVariables();
			for (int j = 0; j < prob.getNumVariables(); j++) {
				if (Utils.getRandNumber(0.0, Math.nextUp(1.0)) < this.probMuta) {

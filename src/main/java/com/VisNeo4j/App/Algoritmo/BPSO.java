...
public class BPSO {
	...
	public BPSO (Problema problema, BPSOParams params, String proyecto, BPSOOpciones opciones) throws FileNotFoundException, IOException, CsvException {
		...
		//this.c1c2Temp = new C1_C2(params.getC1(), 0.0, 0.5, params.getC2(), Constantes.nombreC1_C2UpdateTimeVaryingExponential);
	...
	public Individuo ejecutarBPSO() throws FileNotFoundException, IOException, CsvException {
		while (!this.params.condicionParadaConseguida(this.poblacionPartículas, this.Gbest)){
			...
			//System.out.println(this.c1c2Temp.updateC1(this.params.getMax_Num_Iteraciones(), this.params.getIteracionActual()));
			//System.out.println(this.c1c2Temp.updateC2(this.params.getMax_Num_Iteraciones(), this.params.getIteracionActual()));
			...
			//System.out.println(this.params.getIteracionActual() + ": " + poblacionPartículas.getPoblacion().get(0));
	...
	private Double calcularPendienteFuncion() {
		...
		return this.minPendiente + distMaxMinPendientes * (1 - (sumaTimers0 / this.poblacionPartículas.getNumIndividuos()));
	}
	...
	private Double calcularPendienteFuncion2(int id) {
		...
		return this.minPendiente + distMaxMinPendientes * (1 - (sumaTimers0 / this.poblacionPartículas.getPoblacion().get(id).getVariables().size()));
	}
	...
	private Double calcularPendienteFuncion3() {
		...
		/*if(this.params.getIteracionActual() < this.params.getMax_Num_Iteraciones()/5) {
			...
			for(int i = 0; i < this.poblacionPartículas.getPoblacion().size(); i++) {
				if(Utils.distanciaHamming(this.poblacionLbest.getPoblacion().get(i).getVariables(), this.poblacionPartículas.getPoblacion().get(i).getVariables()) < this.maxDistH) {
					sumaDistL++;
				}
			}
			if(sumaDistL != 0) {
			...
			//System.out.println(this.timers);
			double distMaxMinPendientes = this.maxPendienteL - this.minPendienteL;
			
			return this.minPendienteL + distMaxMinPendientes * (1 - (sumaDistL / this.poblacionPartículas.getNumIndividuos()));
		}else {*/
			...
			for(int i = 0; i < this.poblacionPartículas.getPoblacion().size(); i++) {
				if(Utils.distanciaHamming(this.Gbest.getVariables(), this.poblacionPartículas.getPoblacion().get(i).getVariables()) < this.maxDistH) {
					sumaDistH++;
				}
			}
			if(sumaDistH != 0) {
	...
	private Individuo obtenerLbest(Individuo ind) {
		...
		for(Individuo i : this.poblacionPartículas.getPoblacion()) {
			if(i.getObjetivos().get(0) < Lbest.getObjetivos().get(0) && Utils.distanciaHamming(i.getVariables(), Lbest.getVariables()) < this.neighborhood) {
	...
	public List<Integer> DistanciasLbest(){
		...
		for(int i = 0; i < this.poblacionPartículas.getPoblacion().size(); i++) {
			int distH = Utils.distanciaHamming(this.poblacionPartículas.getPoblacion().get(i).getVariables(), this.poblacionLbest.getPoblacion().get(i).getVariables());
